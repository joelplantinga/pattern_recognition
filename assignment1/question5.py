...
def rescale_img(img):
    ...
    img = cv2.resize(img.astype(float), (14, 14), interpolation=cv2.INTER_AREA)
    return img.flatten()
...
if __name__ == "__main__":
    ...
    def objective(trial):
        ...
        logistic = LogisticRegression(solver= 'saga', random_state=0, C=C, penalty='l2', max_iter= 200, tol=0.01)
    ...
    def objectiveSVM(trial):
        ...
        support_vector_machines = SVC(C=C, gamma=gamma, class_weight='balanced')
    ...
    found_paramSVM.to_csv(f"results/tuning_SVM1.csv")
